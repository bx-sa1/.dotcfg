...
case $cmd in
  create)
    tabbed=$(tabbed -c -d)
    wid=$1; shift
    xdotool windowreparent $wid $tabbed > /dev/null
    echo $tabbed
    ;;
	attach)
    tabbed=$1; shift
		wid=$1; shift
    [ "$(get_class $tabbed)" != "tabbed" ] \
    && notify-send "tabc.sh" "Not an instance of tabbed" \
    || xdotool windowreparent $wid $tabbed
		;;
	detach)
		tabbed=$1; shift
    head=$(get_clients $tabbed | head -n1)
		xdotool windowreparent $head $(get_root_wid) > /dev/null
    echo $head
		;;
  printclass)
		wid=$1; shift
